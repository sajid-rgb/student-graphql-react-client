...
const Barrier = () => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => setIsTheme(true), 2000);
    return () => {
  ...
  return (
    <div className={isTheme ? 'row main' : 'row bg-white'} style={{ height: '100vh' }}>
      <div className="col-md-7 d-flex align-items-center justify-content-center" data-aos="fade-right">
        <img src={HomeImg} className="img-fluid" style={{ width: '90%' }} alt="home" />
      </div>
      <div className="col-md-5 d-flex flex-column  justify-content-center mt-md-2 mt-0" data-aos="fade-left">
        ...
        <div className="ms-md-2 ms-4">
          <Link to="/student">
            {' '}
            <button type="button" className="btn btn-primary mt-3 ms-5 w-50">Go Main Page</button>
          </Link>
          <Link to="/input"><button type="button" className="btn btn-secondary mt-3 ms-5 w-50">Create User Page </button></Link>
        </div>
      </div>
    </div>
